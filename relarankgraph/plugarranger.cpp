                            ...
                            const int rowCount, const int columnCount)
{
    ...
    QVector < qreal > deltaTable =
        QVector < qreal > (rowCount * (columnCount - 1));
    ...
    for (int leftIndex = 0; leftIndex < rowCount; ++leftIndex) {
        for (int rightIndex = leftIndex + 1; rightIndex < rowCount;
             ++rightIndex) {
            qreal currentSum =
                costTable[(columnCount * leftIndex) + guess[leftIndex]] +
                costTable[(columnCount * rightIndex) + guess[rightIndex]];
            qreal swapSum =
                costTable[(columnCount * leftIndex) + guess[rightIndex]] +
